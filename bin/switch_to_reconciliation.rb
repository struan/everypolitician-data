...
POPOLO_FILE = 'ep-popolo-v1.0.json'.freeze
INSTRUCTIONS = 'sources/instructions.json'.freeze
RECONCILIATION = 'sources/reconciliation/wikidata.csv'.freeze
...
(wd = instructions[:sources].find { |i| i[:type] == 'wikidata' }) ||
...
mapped = popolo.persons.select(&:wikidata).sort_by(&:id).map { |p| "#{p.wikidata},#{p.id}" }
